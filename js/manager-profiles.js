(function(){
  ...
  async function render(name){
    ...
    try{
      ...
      const sr=data.seasonRange||[],pr=data.playerRange||[];
      const seasonRangeLabel=Array.isArray(sr)&&sr.length===2?`${sr[0]}–${sr[1]}`:'';
      const playerRangeLabel=Array.isArray(pr)&&pr.length===2?`${pr[0]}–${pr[1]}`:'';
      ...
      node.innerHTML=`
        <section class="manager-profile-section"><div class="manager-profile-head"><div><span>LEAGUE RÉSUMÉ</span><h3>Career Snapshot</h3></div><small>${seasonRangeLabel?seasonRangeLabel+' archive':'Full archive'}</small></div>
