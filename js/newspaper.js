(function initializeNewspaperModule(){
  ...
  async function loadHomepageFeature(){
    ...
    try{
      ...
      window.gateFreshness?.setSourceLabel(status,{source:'Weekly edition',state:data.status==='live'?'live':'final',iso:data.updated_at||data.generated_at,detail:data.status==='live'?'verified live':'verified final'});
