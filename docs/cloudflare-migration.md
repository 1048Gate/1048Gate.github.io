...
| Newspaper | `#newspaper` loads one verified championship recap for every completed season from `historical_archive.json`; `#weekly` loads finalized weekly editions. These are hash views, not separate HTML pages. |
