...
const expectedExportSource = exportFixture.source_status === 'verified_live' ? 'ESPN LIVE SNAPSHOT' : 'ESPN FINAL';
const exportSourceLines = drawnText.filter(entry => String(entry.text).startsWith(`SOURCE: ${expectedExportSource}  ·  PULLED `));
assert.equal(exportSourceLines.length, fullCanvas.height / 1398, 'Every exported page needs an ESPN source and pull-time line matching the edition status.');
