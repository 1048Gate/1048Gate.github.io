 ...
 for(const state of ['live','recap','offseason'])for(const width of [1440,390])for(const theme of ['light','dark']){
  ...
  const metrics=await page.evaluate(()=>{
    ...
    return {theme:document.documentElement.dataset.theme,state:document.querySelector('#home').dataset.homeState,viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth,
      ...
      overflowing:[...document.querySelectorAll('#home *')].filter(n=>{const r=n.getBoundingClientRect();return r.width&&r.right>innerWidth+1}).map(n=>({tag:n.tagName,class:n.className,text:n.textContent.slice(0,90)})),
  ...
  report.cases.push({name,metrics,anchors,a11y,errors});
  writeFileSync(path.join(output,'report.json'),JSON.stringify(report,null,2));
  console.log(name,JSON.stringify({overflow:metrics.scrollWidth>width,anchors,contrastNodes:a11y.violations.reduce((a,v)=>a+v.nodes.length,0),errors}));
  await context.close();
 }
